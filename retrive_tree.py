...
from utils_custom import (reverse_mapping, get_node_list, get_embeddings,
                          distances_from_embeddings, indices_of_nearest_neighbors_from_distances,
                          get_text)
...
class TreeRetriever(BaseRetriever):
    ...
    def __init__(self, config, tree) -> None:
        ...
        self.tree = tree
        # num_layers(traverse), start layer를 별도로 정하지 않으면(None) 
        # tree의 num_layers + 1, tree의 num_layers로 각각 설정    
        self.num_layers = (
    ...
    # query에 대해서 관련된 node를 collapse_tree에서 탐색-> 관련 노드, context 반환
    # token 수가 max_tokens를 넘어가면 중단하는 로직은 주석처리
    ...
    def retrieve(
        ...
        # 여기서 num_layers는 tree의 num_layers가 아니며 travers의 갯수이다.
        # 또한 leaf node가 layer 0이기 때문에 num_layers는 start_layer+1보다 작아야 한다.
